...
if (isset($_POST['register'])){
    ...
    if ($user_name || $email){
        $check_sql = "SELECT user_id from mesr_user WHERE deleted_yn = 'N' AND (user_name = '$user_name' OR email = '$email') ";
        $check_res = $conn->query($check_sql);
        if($check_res->num_rows > 0){
            $form_good = FALSE;
            $validation .= "<p>Sorry, username or email already taken. Please try another.</p>";
        }
    }

    if ($form_good == TRUE){
        $encrypted = password_hash($upassword, PASSWORD_DEFAULT);
        $sql = "INSERT INTO mesr_user ( user_name, password, email)
                VALUES ('$user_name', '$encrypted', '$email')";
        if($conn->query($sql)){
